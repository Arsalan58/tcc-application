      ...
      {tabs.map(t => {
        ...
        return (
          <button key={t.id} onClick={() => onNav(t.id)} className="press" style={{ flex: 1, background: "none", border: "none", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
            <div style={{ width: 44, height: 32, display: "flex", alignItems: "center", justifyContent: "center", background: a ? C.primaryFaint : "transparent", borderRadius: 12 }}>
              {t.icon(a)}
            </div>
...
function BookCover({ title = "", size = 60, height = 86 }) {
  ...
  return (
    <div style={{ width: size, height, background: `linear-gradient(150deg,${a},${b})`, borderRadius: 9, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", position: "relative", overflow: "hidden", boxShadow: "3px 4px 14px rgba(0,0,0,0.22),-2px 0 6px rgba(0,0,0,0.15)" }}>
      ...
      <BookOpen size={iconSize} color="rgba(255,255,255,0.85)" style={{ position: "relative", zIndex: 1 }} />
    </div>
  );
}
...
function BookCard({ book, onPress, onFav, isFav }) {
  return (
    <div onClick={() => onPress(book)} className="press lift" style={{ background: C.bgCard, borderRadius: 18, padding: "14px 14px", display: "flex", gap: 13, boxShadow: S_SM, cursor: "pointer", border: `1px solid ${C.border}`, marginBottom: 10 }}>
      ...
      {onFav && (
        <button onClick={e => { e.stopPropagation(); onFav(book.id); }} className="press" style={{ background: "none", border: "none", cursor: "pointer", padding: "4px 2px", flexShrink: 0, alignSelf: "center", transition: "color .2s" }}>
    ...
    <div style={{ flex: 1, display: "flex", flexDirection: "column", background: C.bg, overflowY: "auto" }}>
      <GreenHero>
        <div style={{ paddingTop: 32, paddingBottom: 52, textAlign: "center" }}>
          <div style={{ marginBottom: 14, animation: "float 4s ease-in-out infinite", display:"flex", justifyContent:"center" }}>
            <img src="/tcc.png" height={"150px"} alt="TCC" />
          </div>
    ...
    <div style={{ flex: 1, display: "flex", flexDirection: "column", background: C.bg, overflowY: "auto" }}>
      ...
      <div style={{ flex: 1, padding: "24px 24px" }}>
        ...
        <div onClick={() => setOk(o => !o)} style={{ background: ok ? C.primaryFaint : C.bgMuted, border: `1.5px solid ${ok ? C.primary : C.border}`, borderRadius: 14, padding: "13px 15px", display: "flex", alignItems: "center", gap: 12, cursor: "pointer", marginBottom: 20, transition: "all .2s" }}>
          <div style={{ width: 22, height: 22, borderRadius: 7, border: `2px solid ${ok ? C.primary : C.borderMid}`, background: ok ? C.primary : "transparent", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, transition: "all .2s" }}>
            {ok && <CheckCircle size={14} color="white" />}
          </div>
    ...
    <div style={{ flex: 1, display: "flex", flexDirection: "column", background: C.bg }}>
      ...
      <div style={{ flex: 1, padding: 32, textAlign: "center", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
        <div style={{ marginBottom: 18, animation: "float 3s ease-in-out infinite", display:"flex", justifyContent:"center" }}>
          <div style={{ width: 72, height: 72, background: C.primaryFaint, borderRadius: 22, display:"flex", alignItems:"center", justifyContent:"center", border:`1.5px solid ${C.primaryPale}` }}>
            <Mail size={34} color={C.primary} />
          </div>
        </div>
        ...
        <div style={{ width: "100%" }}>
          <Btn label="Verify & Continue" onClick={() => { if (code.join("").length < 4) { setErr("Enter full code."); return; } onNav("feed"); }} icon={<CheckCircle size={16} color="white"/>}/>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", background: C.bg }}>
      ...
      <div style={{ flex: 1, padding: 32, textAlign: "center", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
        ...
        {!sent && <div style={{ width: "100%" }}>
          <GInput placeholder="email@example.com" value={email} onChange={setEmail} icon={<Mail size={16} color={C.textMuted}/>}/>
          <Btn label="Send Reset Link" onClick={() => email && setSent(true)} disabled={!email} icon={<Send size={16} color="white"/>}/>
        </div>}
        {sent && <div style={{ width: "100%" }}>
          <Btn label="Back to Login" variant="outline" onClick={() => onNav("login")} icon={<ArrowLeft size={16} color={C.primary}/>}/>
        </div>}
      </div>
    </div>
  );
    ...
    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", background: C.bg }}>
      <GreenHero>
        ...
        <div style={{ background: "white", borderRadius: 14, padding: "11px 15px", marginBottom: 12, display: "flex", gap: 10, alignItems: "center", boxShadow: "0 2px 10px rgba(0,0,0,0.14)" }}>
          ...
          {search && <button onClick={() => setSearch("")} style={{ background: "none", border: "none", color: C.textMid, cursor: "pointer", display:"flex", alignItems:"center" }}><X size={16} color={C.textMid}/></button>}
        </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "16px 14px" }}>
        ...
        {filtered.length === 0 && <div style={{ textAlign: "center", padding: "50px 20px", color: C.textMuted, fontFamily: F.body }}>
          <div style={{ display:"flex", justifyContent:"center", marginBottom:12 }}><Package size={40} color={C.textLight}/></div>No books found
        </div>}
    ...
    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", background: C.bg }}>
      <GreenHero>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 0 }}>
          <button onClick={() => onNav("back")} className="press" style={{ background: "rgba(255,255,255,0.15)", border: "1px solid rgba(255,255,255,0.2)", width: 36, height: 36, borderRadius: 11, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", background: C.bg, borderRadius: "22px 22px 0 0", marginTop: -18, padding: "20px 16px 16px" }}>
        <div style={{ background: C.bgCard, borderRadius: 18, padding: "14px 16px", display: "flex", alignItems: "center", gap: 13, marginBottom: 14, boxShadow: S_SM, border: `1px solid ${C.border}` }}>
          <div style={{ width: 44, height: 44, background: `linear-gradient(135deg,${C.primary},${C.primaryMid})`, borderRadius: 14, display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        <div style={{ background: C.bgCard, borderRadius: 16, padding: "12px 16px", marginBottom: 20, display: "flex", alignItems: "center", gap: 10, border: `1px solid ${C.border}`, boxShadow: S_SM }}>
          <div style={{ display: "flex", gap: 2 }}>
            {[1,2,3,4,5].map(i => <Star key={i} size={16} color={i <= Math.round(book.rating) ? C.gold : C.border} fill={i <= Math.round(book.rating) ? C.gold : "none"} />)}
          </div>
          ...
          <span onClick={() => onNav("review")} style={{ marginLeft: "auto", color: C.primary, fontSize: 12, fontFamily: F.body, fontWeight: 700, cursor: "pointer", display:"flex", alignItems:"center", gap:4 }}>
            <Plus size={13} color={C.primary}/> Review
          </span>
        </div>
        ...
        </> : <div style={{ background: C.bgMuted, borderRadius: 14, padding: "16px", textAlign: "center", fontFamily: F.body, color: C.textMuted, fontSize: 14, border: `1px solid ${C.border}`, display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
          <Package size={18} color={C.textLight}/> This book is currently unavailable
        </div>}
      </div>
    </div>
  );
      ...
      <div style={{ overflowY: "auto", flex: 1 }}>
        <GreenHero>
          <div style={{ paddingTop: 16, paddingBottom: 48, textAlign: "center" }}>
            <div style={{ width: 80, height: 80, background: "rgba(255,255,255,0.15)", borderRadius: 24, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 12px", border: "2px solid rgba(255,255,255,0.2)", boxShadow: "0 4px 20px rgba(0,0,0,0.2)" }}>
              <User size={40} color="white" />
            </div>
            <div style={{ color: "white", fontSize: 21, fontWeight: 700, marginBottom: 4, fontFamily: F.display }}>{user.name}</div>
            <div style={{ color: "rgba(255,255,255,0.55)", fontSize: 12, fontFamily: F.body, marginBottom: 20, display:"flex", alignItems:"center", justifyContent:"center", gap:5 }}>
              <MapPin size={11} color="rgba(255,255,255,0.55)"/> @{user.username} · Muscat Community
            </div>
        ...
        <div style={{ background: C.bg, borderRadius: "22px 22px 0 0", marginTop: -20, padding: "20px 16px 20px" }}>
          <div onClick={() => onNav("badges")} className="press lift" style={{ background: `linear-gradient(135deg,${C.primaryFaint},${C.bgCard})`, borderRadius: 18, padding: "15px 18px", display: "flex", alignItems: "center", gap: 14, marginBottom: 18, border: `1.5px solid ${C.primaryPale}`, cursor: "pointer", boxShadow: S_SM }}>
            ...
            <div style={{ background: C.primary, color: "white", fontSize: 11, padding: "5px 12px", borderRadius: 20, fontFamily: F.body, fontWeight: 700, display:"flex", alignItems:"center", gap:4 }}>
              View <ChevronRight size={12} color="white"/>
            </div>
          </div>
    ...
    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", background: C.bg }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: 22 }}>
        <div style={{ textAlign: "center", marginBottom: 26 }}>
          <div style={{ width: 80, height: 80, background: `linear-gradient(135deg,${C.primary},${C.primaryMid})`, borderRadius: 24, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 10px", boxShadow: G_GREEN }}>
            <User size={40} color="white" />
          </div>
          <span style={{ color: C.primary, fontSize: 13, fontFamily: F.body, fontWeight: 600, cursor: "pointer", display:"inline-flex", alignItems:"center", gap:5 }}>
            <Pencil size={13} color={C.primary}/> Change Photo
          </span>
        </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: 14 }}>
        {favBooks.length === 0 ? <div style={{ textAlign: "center", padding: "70px 20px" }}>
          <div style={{ display:"flex", justifyContent:"center", marginBottom:14, animation:"float 3s ease-in-out infinite" }}>
            <Heart size={50} color={C.textLight} />
          </div>
          <div style={{ fontWeight: 700, fontSize: 16, marginBottom: 8, color: C.text, fontFamily: F.display }}>No favourites yet</div>
          <div style={{ fontSize: 13, color: C.textMuted, fontFamily: F.body, display:"flex", alignItems:"center", justifyContent:"center", gap:6 }}>
            Tap <Heart size={14} color="#E74C3C" fill="#E74C3C"/> on any book to save it here
          </div>
        </div>
    ...
    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", background: C.bg }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "16px 14px" }}>
        ...
        <div onClick={() => onNav("chatbot")} className="press lift" style={{ background: `linear-gradient(135deg,${C.primary},${C.primaryMid})`, borderRadius: 20, padding: "18px 20px", display: "flex", alignItems: "center", gap: 16, marginBottom: 12, cursor: "pointer", boxShadow: `${S_MD},${G_GREEN}` }}>
          <div style={{ width: 48, height: 48, background: "rgba(255,255,255,0.15)", borderRadius: 15, display: "flex", alignItems: "center", justifyContent: "center", border: "1px solid rgba(255,255,255,0.2)" }}>
        ...
        <div onClick={() => onNav("myBooks")} className="press lift" style={{ background: C.bgCard, border: `1.5px solid ${C.primaryPale}`, borderRadius: 20, padding: "18px 20px", display: "flex", alignItems: "center", gap: 16, cursor: "pointer", boxShadow: S_SM }}>
          <div style={{ width: 48, height: 48, background: C.primaryFaint, borderRadius: 15, display: "flex", alignItems: "center", justifyContent: "center", border: `1px solid ${C.primaryPale}` }}>
            <ListChecks size={24} color={C.primary} />
          </div>
        ...
        {filtered.map(b => (
          <div key={b.id}>
            <div onClick={() => setExpandId(expandId === b.id ? null : b.id)} style={{ background: C.bgCard, borderRadius: expandId === b.id ? "18px 18px 0 0" : 18, padding: 14, display: "flex", gap: 12, border: `1px solid ${C.border}`, borderBottom: expandId === b.id ? "none" : undefined, marginBottom: expandId === b.id ? 0 : 10, cursor: "pointer", boxShadow: expandId === b.id ? "none" : S_SM }}>
              ...
              <div style={{ color: C.textLight, alignSelf: "center", transition: "transform .2s", transform: expandId === b.id ? "rotate(180deg)" : "none", display:"flex" }}>
    ...
    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", background: C.bg }}>
      <GreenHero>
        <div style={{ display: "flex", alignItems: "center", gap: 13, paddingBottom: 16 }}>
          <div style={{ width: 42, height: 42, background: `linear-gradient(135deg,${C.gold},${C.goldLight})`, borderRadius: 14, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: G_GOLD }}>
        ...
        {messages.map((m, i) => (
          <div key={i} style={{ animation: "fadeUp .25s ease forwards" }}>
            <div style={{ display: "flex", justifyContent: m.from === "user" ? "flex-end" : "flex-start", alignItems: "flex-end", gap: 8 }}>
              {m.from === "bot" && <div style={{ width: 30, height: 30, background: `linear-gradient(135deg,${C.gold},${C.goldLight})`, borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
        ...
        {notifs.map((n, i) => (
          <div key={i} onClick={() => setNotifs(ns => ns.map((nn, ii) => ii === i ? {...nn,unread:false} : nn))} className="press" style={{ background: n.unread ? C.primaryFaint : C.bgCard, borderRadius: 18, padding: "14px 14px", display: "flex", gap: 12, marginBottom: 10, border: `1px solid ${n.unread ? C.primaryPale : C.border}`, cursor: "pointer", boxShadow: S_SM }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 11, color: C.textLight, fontFamily: F.body, marginTop: 5, display:"flex", alignItems:"center", gap:4 }}>
                <Clock size={10} color={C.textLight}/> {n.time}
              </div>
            </div>
    ...
    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", background: C.bg }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: 24, textAlign: "center" }}>
        <div style={{ display:"flex", justifyContent:"center", marginBottom: 16, animation: "float 3s ease-in-out infinite" }}>
          <div style={{ width:72, height:72, background:C.primaryFaint, borderRadius:22, display:"flex", alignItems:"center", justifyContent:"center", border:`1.5px solid ${C.primaryPale}` }}>
            <BookMarked size={36} color={C.primary} />
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 24 }}>
          {[
            ["1,247","Books Listed",<Library size={20} color={C.primary}/>],
            ["342","Active Readers",<Users size={20} color={C.info}/>],
            ["15","Communities",<Globe size={20} color={C.success}/>],
            ["892","Books Shared",<Repeat size={20} color={C.gold}/>],
          ].map(([v,l,icon],i) => <div key={i} style={{ background: C.bgCard, borderRadius: 18, padding: "18px 14px", border: `1px solid ${C.border}`, boxShadow: S_SM }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: 14 }}>
        {faqs.map((f, i) => <div key={i} onClick={() => setOpen(open === i ? null : i)} className="press" style={{ background: C.bgCard, borderRadius: 18, padding: "16px 18px", marginBottom: 10, border: `1.5px solid ${open === i ? C.primary : C.border}`, cursor: "pointer", transition: "all .22s", boxShadow: open === i ? S_MD : S_SM }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <div style={{ transition: "transform .22s", transform: open === i ? "rotate(180deg)" : "none", flexShrink: 0, display:"flex" }}>
              <ChevronDown size={16} color={C.textLight}/>
            </div>
          </div>
