...
import {
  ...
  ListChecks, Trophy, Medal, Gem, Globe, X,
} from "lucide-react";
...
const BOOK_EMOJIS = {
  "The Alchemist":"🏜️","Atomic Habits":"⚡","Sapiens":"🦴","Dune":"🌙",
  "The Midnight Library":"🌌","Educated":"🎓","The Kite Runner":"🪁",
  "1984":"👁️","Pride and Prejudice":"💐","Brave New World":"🧬",
};
...
function BookCard({book,onPress,onFav,isFav}){
  return(
    <div onClick={()=>onPress(book)} className="press lift" style={{background:C.bgCard,borderRadius:18,padding:"14px 14px",display:"flex",gap:13,boxShadow:S_SM,cursor:"pointer",border:`1px solid ${C.border}`,marginBottom:10}}>
      ...
      {onFav&&<button onClick={e=>{e.stopPropagation();onFav(book.id);}} className="press" style={{background:"none",border:"none",cursor:"pointer",padding:"4px 2px",flexShrink:0,alignSelf:"center",transition:"color .2s"}}>
  ...
  return(
    <div style={{marginBottom:14}}>
      ...
      <div style={{position:"relative"}}>
        {icon&&<span style={{position:"absolute",left:13,top:"50%",transform:"translateY(-50%)",lineHeight:1,display:"flex",alignItems:"center"}}>{icon}</span>}
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"40px 30px",position:"relative",zIndex:1}}>
        <div style={{animation:"float 4s ease-in-out infinite",marginBottom:30}}>
          <div style={{width:96,height:96,background:"rgba(255,255,255,0.13)",borderRadius:28,display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 8px 32px rgba(0,0,0,0.3),inset 0 1px 0 rgba(255,255,255,0.15)",border:"1px solid rgba(255,255,255,0.18)"}}>
            <img src="/tcc.png" height={"120px"} alt="TCC"/>
          </div>
        </div>
    ...
    <div style={{flex:1,display:"flex",flexDirection:"column",background:C.bg,overflowY:"auto"}}>
      <GreenHero>
        <div style={{paddingTop:32,paddingBottom:52,textAlign:"center"}}>
          <div style={{marginBottom:14,animation:"float 4s ease-in-out infinite",display:"flex",justifyContent:"center"}}><img src="/tcc.png" height={"150px"} alt="TCC"/></div>
  ...
  return(
    <div style={{flex:1,display:"flex",flexDirection:"column",background:C.bg,overflowY:"auto"}}>
      ...
      <div style={{flex:1,padding:"24px 24px"}}>
        ...
        {err&&<div style={{background:"#FEF2F2",border:"1px solid #FECACA",borderRadius:12,padding:"11px 15px",marginBottom:16,color:C.danger,fontSize:13,fontFamily:F.body,display:"flex",alignItems:"center",gap:8}}><AlertCircle size={15} color={C.danger}/>{err}</div>}
    ...
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",background:C.bg}}>
      <GreenHero>
        ...
        <div style={{display:"flex",gap:8,marginBottom:12}}>
          {[{v:books.length,l:"Community Books",icon:<Library size={18} color="rgba(255,255,255,0.8)"/>},{v:books.filter(b=>b.status==="Available").length,l:"Available Now",icon:<CheckCircle size={18} color="rgba(255,255,255,0.8)"/>}].map((s,i)=>(
        ...
        {!search&&filter==="All"&&featured&&(
          <div onClick={()=>{onBookSelect(featured);onNav("bookDetail");}} className="press lift" style={{background:`linear-gradient(135deg,${C.primary},${C.primaryMid})`,borderRadius:22,padding:"18px 18px",marginBottom:18,cursor:"pointer",display:"flex",gap:16,alignItems:"center",boxShadow:`${S_MD},${G_GREEN}`}}>
            ...
            <div style={{flex:1}}>
              ...
              <div style={{color:C.goldLight,fontSize:12,fontFamily:F.body,fontWeight:600,display:"flex",alignItems:"center",gap:4}}>
                <Star size={12} color={C.goldLight} fill={C.goldLight}/>{featured.rating} · {featured.reviews} reviews
              </div>
            </div>
          </div>
        )}
    ...
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",background:C.bg}}>
      ...
      <div style={{flex:1,overflowY:"auto",background:C.bg,borderRadius:"22px 22px 0 0",marginTop:-18,padding:"20px 16px 16px"}}>
        ...
        {book.status==="Available"?<>
          <Btn label="Request to Borrow" icon={<Send size={16} color="white"/>} onClick={()=>onRequest("Borrow",book.title)}/>
          <Btn label="Take as Gift" variant="outline" icon={<Gift size={16} color={C.primary}/>} onClick={()=>onRequest("Gift",book.title)}/>
          <Btn label="Request to Buy" variant="gold" icon={<ShoppingCart size={16} color="white"/>} onClick={()=>onRequest("Buy",book.title)}/>
...
function ProfileScreen({onNav,user,myBooks}){
  ...
  const menuItems=[
    ...
    {icon:<Info size={17} color={C.info}/>,bg:"#EFF6FF",label:"About TCC Books",action:()=>onNav("about")},
      ...
      <div style={{overflowY:"auto",flex:1}}>
        <GreenHero>
          <div style={{paddingTop:16,paddingBottom:48,textAlign:"center"}}>
            ...
            <div style={{color:"rgba(255,255,255,0.55)",fontSize:12,fontFamily:F.body,marginBottom:20,display:"flex",alignItems:"center",justifyContent:"center",gap:5}}><MapPin size={11} color="rgba(255,255,255,0.55)"/>@{user.username} · Muscat Community</div>
    ...
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",background:C.bg}}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:14}}>
        {favBooks.length===0?<div style={{textAlign:"center",padding:"70px 20px"}}>
          ...
          <div style={{fontSize:13,color:C.textMuted,fontFamily:F.body,display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>Tap <Heart size={14} color="#E74C3C" fill="#E74C3C"/> on any book to save it here</div>
        </div>
...
function DashboardScreen({onNav,myBooks}){
  const stats=[
    [myBooks.length,"Listed",<BookOpen size={22} color={C.primary}/>,C.primary],
    [myBooks.filter(b=>b.status==="Borrowed").length,"Borrowed Out",<Repeat size={22} color={C.info}/>,C.info],
    [myBooks.filter(b=>b.status==="Gifted").length,"Gifted",<Gift size={22} color={C.success}/>,C.success],
    [myBooks.filter(b=>b.status==="Available").length,"Available",<CheckCircle size={22} color={C.warning}/>,C.warning],
    [2,"Received",<Package size={22} color={C.gold}/>,C.gold],
    [0,"Sold",<DollarSign size={22} color={C.danger}/>,C.danger],
  ];
  return(
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",background:C.bg}}>
        ...
        {filtered.map(b=>(
          <div key={b.id}>
            <div onClick={()=>setExpandId(expandId===b.id?null:b.id)} style={{background:C.bgCard,borderRadius:expandId===b.id?"18px 18px 0 0":18,padding:14,display:"flex",gap:12,border:`1px solid ${C.border}`,borderBottom:expandId===b.id?"none":undefined,marginBottom:expandId===b.id?0:10,cursor:"pointer",boxShadow:expandId===b.id?"none":S_SM}}>
              ...
              <div style={{color:C.textLight,alignSelf:"center",transition:"transform .2s",transform:expandId===b.id?"rotate(180deg)":"none",display:"flex"}}><ChevronDown size={16} color={C.textLight}/></div>
            </div>
...
function NotificationsScreen({onNav,notifs,setNotifs}){
  const iconMap={
    "New Message":<MessageSquare size={20} color={C.primary}/>,
    "Request Accepted":<CheckCircle size={20} color={C.success}/>,
    "New Book Listed":<BookOpen size={20} color={C.info}/>,
    "Return Reminder":<Clock size={20} color={C.warning}/>,
    "Gift Available":<Gift size={20} color={C.gold}/>,
    "Borrow Request Sent":<Send size={20} color={C.primary}/>,
    "Gift Request Sent":<Gift size={20} color={C.gold}/>,
  };
  return(
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",background:C.bg}}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:14}}>
        {notifs.map((n,i)=>(
          <div key={i} onClick={()=>setNotifs(ns=>ns.map((nn,ii)=>ii===i?{...nn,unread:false}:nn))} className="press" style={{background:n.unread?C.primaryFaint:C.bgCard,borderRadius:18,padding:"14px 14px",display:"flex",gap:12,marginBottom:10,border:`1px solid ${n.unread?C.primaryPale:C.border}`,cursor:"pointer",boxShadow:S_SM}}>
            <div style={{width:44,height:44,background:n.unread?C.primaryPale:C.bgMuted,borderRadius:14,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
              {iconMap[n.title]||<Bell size={20} color={C.textMuted}/>}
            </div>
            <div style={{flex:1}}>
...
function BadgesScreen({onNav,myBooks}){
  const cnt=myBooks.length;
  const badges=[
    {icon:<Medal size={34} color="#CD7F32"/>,name:"Bronze Reader",range:"1–5 books",earned:cnt>=1,threshold:1},
    {icon:<Medal size={34} color="#AAA"/>,name:"Silver Reader",range:"6–20 books",earned:cnt>=6,threshold:6},
    {icon:<Trophy size={34} color={C.gold}/>,name:"Gold Reader",range:"21–50 books",earned:cnt>=21,threshold:21},
    {icon:<Gem size={34} color="#60a5fa"/>,name:"Diamond Reader",range:"51+ books",earned:cnt>=51,threshold:51},
  ];
    ...
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",background:C.bg}}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:16}}>
        ...
        {badges.map((b,i)=><div key={i} style={{background:b.earned?C.bgCard:C.bgMuted,borderRadius:18,padding:"16px 18px",display:"flex",alignItems:"center",gap:14,marginBottom:12,border:`1.5px solid ${b.earned?C.goldBorder:C.border}`,opacity:b.earned?1:.5,boxShadow:b.earned?S_SM:"none"}}>
          ...
          {b.earned
            ?<div style={{background:C.primaryFaint,color:C.primary,fontSize:11,padding:"5px 12px",borderRadius:20,fontFamily:F.body,fontWeight:700,display:"flex",alignItems:"center",gap:5}}><CheckCircle size={12} color={C.primary}/> Earned</div>
            :<div style={{background:C.bgWarm,color:C.textLight,fontSize:11,padding:"5px 12px",borderRadius:20,fontFamily:F.body,display:"flex",alignItems:"center",gap:5}}><Lock size={12} color={C.textLight}/> Locked</div>}
        </div>)}
      </div>
    </div>
  );
    ...
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",background:C.bg}}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:24,textAlign:"center"}}>
        <div style={{display:"flex",justifyContent:"center",marginBottom:16,animation:"float 3s ease-in-out infinite"}}>
          <div style={{width:72,height:72,background:C.primaryFaint,borderRadius:22,display:"flex",alignItems:"center",justifyContent:"center",border:`1.5px solid ${C.primaryPale}`}}><BookMarked size={36} color={C.primary}/></div>
        </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:24}}>
          {[["1,247","Books Listed",<Library size={20} color={C.primary}/>],["342","Active Readers",<Users size={20} color={C.info}/>],["15","Communities",<Globe size={20} color={C.success}/>],["892","Books Shared",<Repeat size={20} color={C.gold}/>]].map(([v,l,icon],i)=>(
            <div key={i} style={{background:C.bgCard,borderRadius:18,padding:"18px 14px",border:`1px solid ${C.border}`,boxShadow:S_SM}}>
        ...
        <Btn label="Contact Us" variant="outline" icon={<Mail size={16} color={C.primary}/>} onClick={()=>{}}/>
      </div>
    </div>
  );
}
...
function FAQsScreen({onNav}){
  ...
  const faqs=[
    ...
    {q:"How do I save favourite books?",a:"Tap the heart icon on any book to add it to your Favourites list in your Profile."},
  ];
  return(
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",background:C.bg}}>
...
export default function Light(){
  ...
  const [notifs,setNotifs]=useState([
    ...
    {title:"New Book Listed",body:"Ali listed 'Educated' by Tara Westover",time:"3h ago",unread:false},
  ...
  const handleRequest=(type,title)=>{
    ...
    setNotifs(ns=>[{title:`${type} Request Sent`,body:`Requested to ${type.toLowerCase()} "${title}"`,time:"just now",unread:false},...ns]);
  };
